...
test('test', async ({ page }) => {
 await login(page);  // LoginHelper.ts file yaha call ho raha hai
  ...
  const email = `akashat${randomNum}@remittanceshub.com`;
...
//   
  ...
  // await page.pause();
  const randomPhone = '982900' + Math.floor(1000 + Math.random() * 9000);
